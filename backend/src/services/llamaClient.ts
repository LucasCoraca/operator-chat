...
export interface LlamaConfig {
  baseUrl: string;
  apiKey?: string;
  // The following fields are kept for backward compatibility but are no longer sent to the server
  model?: string;
  temperature?: number;
  maxTokens?: number;
  topP?: number;
...
export class LlamaClient {
  ...
  private getNativeBaseUrl(): string {
  ...
  async countTokens(content: string): Promise<number> {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json() as { tokens_evaluated?: number; prompt_n?: number };
  ...
  async countChatTokens(messages: ChatMessage[]): Promise<number> {
  ...
  async chatStream(
    ...
    options?: {
      onReasoningToken?: (token: string) => void; // Separate callback for reasoning_content
