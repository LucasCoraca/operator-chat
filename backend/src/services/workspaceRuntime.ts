...
function ensureInsideLocalRoot(root: string, relativePath: string): string {
  const resolvedRoot = path.resolve(root);
  const resolvedPath = path.resolve(resolvedRoot, relativePath === '.' ? '' : relativePath);
...
abstract class BaseWorkspaceRuntime implements WorkspaceRuntime {
  ...
  async editFile(relativePath: string, oldString: string, newString: string, replaceAll = false): Promise<string> {
  ...
  async applyPatch(patchText: string): Promise<string> {
  ...
  protected async readRawFile(relativePath: string): Promise<string> {
    return this.readFile(relativePath, { offset: 1, limit: Number.MAX_SAFE_INTEGER });
  }
}

export class LocalWorkspaceRuntime extends BaseWorkspaceRuntime {
  readonly kind = 'local_sandbox' as const;
  readonly root: string;

  constructor(private sandboxManager: SandboxManager, private sandboxId: string) {
    super();
  ...
  async readFile(relativePath: string, options: ReadOptions = {}): Promise<string> {
    const normalized = normalizeRelativePath(relativePath);
    const fullPath = ensureInsideLocalRoot(this.root, normalized);
  ...
  async exec(options: CommandOptions): Promise<CommandResult> {
...
export class SshWorkspaceRuntime extends BaseWorkspaceRuntime {
  readonly kind = 'ssh_remote' as const;
  readonly root: string;

  constructor(private config: SshWorkspaceConfig) {
    super();
    ...
    this.root = path.posix.normalize(config.root);
  }

  async list(relativePath = '.'): Promise<string> {
    const target = this.remotePath(relativePath);
    const result = await this.execRaw(
      `if [ ! -d ${shellQuote(target)} ]; then echo "Not a directory: ${target}" >&2; exit 2; fi\n` +
...
`;
  ...
  async readTerminal(terminalId: string, tailLines = DEFAULT_TERMINAL_TAIL_LINES, maxBytes = DEFAULT_TERMINAL_MAX_BYTES): Promise<string> {
    ...
    const dir = path.posix.join(this.terminalRoot(), id);
    const script = `
dir=${shellQuote(dir)}
if [ ! -d "$dir" ]; then
...
`;
  ...
  async killTerminal(terminalId: string): Promise<string> {
    const id = normalizeTerminalId(terminalId);
    const dir = path.posix.join(this.terminalRoot(), id);
    const script = `
dir=${shellQuote(dir)}
...
`;
  ...
  private remotePath(relativePath: string): string {
    ...
    return path.posix.join(this.root, normalized === '.' ? '' : normalized);
  }

  private sshArgs(privateKeyPath?: string): string[] {
    const args = [
      '-p',
  ...
  private async execRaw(script: string, timeoutMs: number): Promise<CommandResult> {
    ...
    try {
      ...
      if (tempKeyPath) {
        fs.rmSync(path.dirname(tempKeyPath), { recursive: true, force: true });
      }
    }
  }

  private terminalRoot(): string {
    return path.posix.join(this.root, '.operator-chat', 'terminals');
  }

  private async execManaged(command: string, workdir: string, timeoutMs: number): Promise<CommandResult> {
    const terminalId = randomUUID();
    const terminalDir = path.posix.join(this.terminalRoot(), terminalId);
...
function runProcess(
  ...
  return new Promise((resolve) => {
    ...
    child.on('error', (error) => {
      clearTimeout(timer);
      const message = (error as NodeJS.ErrnoException).code === 'ENOENT' && command === 'ssh'
...
function normalizeTerminalId(input: string): string {
...
function extractManagedSection(output: string, header: string, nextHeader?: string): string {
...
function parseManagedCommandOutput(output: string): {
...
function writeTempPrivateKey(privateKey: string): string {
  const dir = fs.mkdtempSync(path.join('/tmp', 'operator-chat-ssh-'));
...
function formatReadOutput(filePath: string, content: string, options: ReadOptions): string {
  const offset = Math.max(1, Number(options.offset || 1));
  const limit = Math.max(1, Math.min(MAX_READ_LIMIT, Number(options.limit || DEFAULT_READ_LIMIT)));
...
function parsePatch(patchText: string): PatchChange[] {
...
function applyPatchChunks(oldContent: string, chunks: string[]): string {
