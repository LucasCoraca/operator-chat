...
export async function initializeDatabase(): Promise<void> {
  try {
    ...
    try {
      await pool.execute(
        `ALTER TABLE agent_run_tasks MODIFY COLUMN status ENUM('pending', 'in_progress', 'completed', 'cancelled') NOT NULL DEFAULT 'pending'`
      );
    } catch (error) {
    ...
    try {
      await pool.execute(
        `ALTER TABLE agent_run_tasks ADD COLUMN priority ENUM('high', 'medium', 'low') NOT NULL DEFAULT 'medium' AFTER status`
      );
    } catch (error) {
      await ignoreDuplicateMigration(error);
    }
